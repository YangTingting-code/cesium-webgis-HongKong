...
//     stack: dist,
//     barMaxWidth: 20,
//     itemStyle: { color: dist === '九龙城区' ? '#36A69F' : '#789BC3', barBorderRadius: 0 },
...
//       }
//     },   // 蓝色（人流减少）,
//     label: { show: true, position: dist === '九龙城区'? "top" : [-5,-10], formatter: (p:any)=>p.value>0?'▼'+Math.round(p.value):'' ,fontSize:"0.7rem",
//       color:'#4FC3F7',
...
// //     // 功能区
// //     const area = params[0].data.group || params[0].name;
// //     let html = `<div><b>${area}</b></div>`;

// //     districts.forEach(dist => {
// //       const item = params.find(p => p.data.district === dist);
// //       if (item) {
// //         const { prev, curr, diff } = item.data;
...
// //     districts.forEach(dist => {
// //       const items = params.filter(p => p.data.district === dist);
// //       if (!items.length) return;

// //       // 行政区标题
// //       htmlArr.push(`<div style="margin-bottom:4px;font-weight:bold;">${dist}</div>`);

// //       items.forEach(item => {
// //         const { group, prev, curr, diff } = item.data;
// //         const diffText = diff >= 0 ? `▲${Math.round(diff)}` : `▼${Math.round(-diff)}`;
// //         const diffColor = diff > 0 ? '#FF8A65' : diff < 0 ? '#4FC3F7' : '#eee';
// //         const circleColor = dist === '九龙城区' ? '#36A69F' : '#789BC3';

// //         htmlArr.push(`
// //           <div style="display:flex;align-items:center;margin-left:8px;padding:2px 0;border-bottom:1px solid rgba(255,255,255,0.1);">
// //             <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${circleColor};margin-right:6px;"></span>
// //             <span style="flex:1;">${group}</span>
// //             <span style="margin-left:6px;color:${diffColor};font-weight:bold;">${diffText}</span>
// //           </div>
// //         `);
// //       });
// //     });
...
/* ②③ 辅助系列：名字随便，但默认隐藏 */
  ...
  /* --- 2. 图例只保留 2 项（用户看到的）--- */
  // const legendData = districts.map(dist => ({ name: dist, icon: 'roundRect' }))
  /* ---------- 1. 图例按钮 = 真实系列名 ---------- */
// const legendData = [
//   ...districts.map(d => ({ name: `${d}-主系列`, icon: 'roundRect' })),
//   ...districts.map(d => ({ name: `${d}-人流减少`, icon: 'roundRect' })),
//   ...districts.map(d => ({ name: `${d}-人流增加`, icon: 'roundRect' }))
// ]
// /* ---------- 2. 初始只显示“主系列” ---------- */
// const legendSelected: Record<string, boolean> = {}
// districts.forEach(d => {
//   legendSelected[`${d}-主系列`] = true
//   legendSelected[`${d}-人流减少`] = false
//   legendSelected[`${d}-人流增加`] = false
// })
...
//   const legendSelected: Record<string, boolean> = {}
//   // districts.forEach(dist => {
//   //   legendSelected[dist] = true // 图例按钮状态
//   // })
//   districts.forEach(dist => {
//   legendSelected[`${dist}-主系列`] = true              // 只让主系列出现
//   legendSelected[`${dist}-人流减少`] = true
//   legendSelected[`${dist}-人流增加`] = true           // 注意你这里前缀是“人流增加-”
// })
  areas.forEach((a)=>{
    districts.forEach(dist=>{
      const prev = map[`${dist}|${prevP}|${a}`] || 0
...
onMounted(async () => {
  ...
  inst.on('legendselectchanged', ({ name, selected }: any) => {
    ...
    inst.setOption({ legend: { selected: toSet } })
  })
})
